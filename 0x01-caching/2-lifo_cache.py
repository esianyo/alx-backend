#!/usr/bin/env python3
""" LIFOCache class for LIFO caching """
...
class LIFOCache(BaseCaching):
    ...
    and implements a LIFO (Last-In-First-Out)
    caching strategy.
    """
    ...
    def put(self, key, item):
        ...
        if key is not None and item is not None:
            ...
            self.cache_order.append(key)  # Add key to the order list (LIFO)

            # Enforce maximum items limit (LIFO eviction)
            if len(self.cache_order) > self.MAX_ITEMS:
                # Discard the newest item (last in)
